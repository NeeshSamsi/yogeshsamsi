...
_merge_ with Tailwind's defaults is correct for v3, but doesn't actually
...
the _other_ colours in `tailwind.config.js` (`background`, `foreground`,
...
| `tailwind-hamburgers`                    | **Superseded — removed in D2 (todo 186), before this migration.** No action needed here.           | **[VERIFIED - repo]** D2 replaced the plugin's ~40 lines of generated CSS with an equivalent hand-written block in `globals.css` (`.tham` / `.tham-box` / `.tham-inner` / `.tham-active`, timings and sizes copied exactly from the plugin's `spin`/`w-6` output) and dropped the dependency and its `require()` from `tailwind.config.js` entirely. There is no `@plugin` line to add for it, and its v3→v4 compatibility is now moot since it is no longer in the tree. The `mobile-nav-open` screenshot still matters for this migration — it now verifies that the plain-CSS replacement (not the plugin) survives the config rewrite intact — but the plugin-compatibility coin-flip this row originally described no longer applies.                                                                                                                                                                                                                                                                                                                                  |
